...
public class MinBinHeap implements Heap_Interface {
    ...
    private void bubbleUp(int idx) {
        ...
        if (array[idx].getDistance() < array[par].getDistance()) {
    ...
    private void bubbleDown(int idx) {
        int swp = getLeft(idx);
        if(swp>9999)return;
        if (array[getRight(idx)] != null && array[getRight(idx)].getDistance() < array[swp].getDistance())
            swp = getRight(idx);
        if (array[idx].getDistance() > array[swp].getDistance()) {
